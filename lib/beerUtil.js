 ...
 * @param {number} last Optional parameter indicating the number of "latest" results to return
 */
var getUsedBeers = function (callback, last) {
    ...
    storage.read(USED_BEERS_STORAGE, function (error, data) {
        if (error) {
            ...
            try {
                ...
                if (last && last > 0 && last < beers.length) {
                    console.log("last : " + JSON.stringify(beers));
                    callback(null, beers.slice(beers.length - last, beers.length));
                }
                else {
                    console.log("calling back with beers : " + JSON.stringify(beers));
                    callback(null, beers);
                }
            }
            catch (e) {
                console.log("calling back with empty");
                callback(null, []);
            }
        }
    });
